...
class MainActivity : AppCompatActivity() {   // 声明主 Activity 类，继承自 AppCompatActivity，用于兼容旧版 Android
    ...
    override fun onCreate(savedInstanceState: Bundle?) {  //重写 Activity 的生命周期方法 onCreate，应用启动时调用。
        ...
        val navView: BottomNavigationView = binding.navView  // 获取 BottomNavigationView 的引用，用于显示底部导航栏。
        ...
        navView.setupWithNavController(navController)  // 将 BottomNavigationView 与 NavController 关联，使得底部导航栏可以响应导航操作。
    }
}