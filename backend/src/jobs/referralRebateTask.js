...
class ReferralRebateTask {
  ...
  static async calculateAndDistributeRebate(referrerId) {
    ...
    for (const sub of subordinates) {
      // 查该下级上一次被计入返利的时间
      const [[lastRebateRow]] = await pool.query(
    ...
    if (rebateAmount <= 0) return 0;

    // 4. 获取推荐人当前余额和邀请码
    const [referrerStatus] = await pool.query(
      'SELECT current_bitcoin_balance FROM user_status WHERE user_id = ?',
      [referrerId]
    );
    if (referrerStatus.length === 0) throw new Error('推荐人不存在');
    ...
    // 6. 记录每个下级的返利明细，同时写入交易记录
    let runningBalance = parseFloat(referrerStatus[0].current_bitcoin_balance) + rebateAmount;
    let accumulatedRebate = 0;

    for (const sub of subordinateDetails) {
      const subRebate = sub.revenue * 0.20;
      accumulatedRebate += subRebate;
      const balanceAtThisPoint = parseFloat(referrerStatus[0].current_bitcoin_balance) + accumulatedRebate;
      ...
      // bitcoin_transaction_records 每条下级独立一行
      await pool.query(`
      ...
      `, [referrerId, subRebate, balanceAtThisPoint, `From: ${sub.userId}`]);
    }
