...
function authenticateToken(req, res, next) { // 定义中间件函数（req：请求、res：响应、next：下一个处理器）
  ...
  const secret = process.env.JWT_SECRET; // 读取服务端用于签名/校验的密钥（必须配置），验证 token 必须要它
  if (!secret) { // 若未配置密钥，属于服务端环境问题
    return res.status(500).json({ error: '服务器配置错误：缺少 JWT_SECRET' }); // 返回 500，提示服务端缺少配置
  }
