...
class CheckInService {
  ...
  static async initRewardConfig() {
    try {
      const [rows] = await db.query(
        'SELECT consecutive_days, points_reward, bonus_multiplier, description FROM check_in_reward_config WHERE is_active = TRUE ORDER BY consecutive_days ASC'
      );
    ...
    for (const [days, config] of Object.entries(this.MILESTONE_REWARDS)) {
      ...
      if (consecutiveDays === targetDays) {
        ...
        const [existingMilestone] = await connection.query(
          'SELECT id FROM user_check_in WHERE user_id = ? AND consecutive_days >= ? AND milestone_claimed = TRUE AND milestone_type = ?',
  ...
  static async checkInOld(userId) {
    ...
    try {
      ...
      const [lastCheckIn] = await connection.query(
        'SELECT check_in_date, consecutive_days FROM user_check_in WHERE user_id = ? ORDER BY check_in_date DESC LIMIT 1',
        [userId]
      );
      ...
      // 4. 计算每日签到加成过期时间 (2小时后)
      const bonusExpireTime = new Date(Date.now() + 2 * 60 * 60 * 1000);
      ...
      await connection.query(
        ...
        [userId, today, consecutiveDays, pointsEarned, bonusExpireTime, milestoneType, milestoneClaimed]
      );
      ...
      if (redisClient.isReady()) {
        // 缓存签到状态
        await redisClient.cacheCheckInStatus(userId, today, consecutiveDays, true, bonusExpireTime.toISOString());

        // 添加到每日签到加成激活用户集合
        const expireTimestamp = bonusExpireTime.getTime();
      ...
      return {
        ...
        dailyBonusExpire: bonusExpireTime.toISOString(),
  ...
  static async getCheckInStatus(userId) {
    try {
      ...
      let bonusActive = false;
      let bonusTimeRemaining = 0;
      if (lastCheckIn.bonus_expire_time) {
        const expireTime = new Date(lastCheckIn.bonus_expire_time);
        const now = new Date();
        bonusActive = expireTime > now;
        bonusTimeRemaining = bonusActive ? Math.floor((expireTime - now) / 1000) : 0;
      }
      ...
      const result = {
        ...
        dailyBonusActive: bonusActive,
        dailyBonusExpire: lastCheckIn.bonus_expire_time,
        dailyBonusTimeRemaining: bonusTimeRemaining,
        nextReward: this.calculateReward(hasCheckedInToday ? lastCheckIn.consecutive_days + 1 : 1)
      };
  ...
  static async getCheckInHistory(userId, days = 30) {
    try {
      ...
      return {
        ...
        maxConsecutiveDays: records.length > 0 ? Math.max(...records.map(r => r.consecutive_days)) : 0
      };
