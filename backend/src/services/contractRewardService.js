...
class ContractRewardService {
    ...
    try {
      const [users] = await pool.query(`
        ...
        FROM (
          ...
          WHERE (
            (mining_status = 'mining' AND free_contract_end_time > ?) 
            OR (mining_status = 'completed' AND free_contract_end_time >= ? AND free_contract_end_time <= ?)
          )
      ...
      `, [startTime, startTime, endTime, startTime, startTime, endTime]);
  ...
  static async calculateFreeContractRevenue(userId, contractType, startTime, endTime, speedPerSecond) {
    try {
      ...
      `, [userId, contractType, startTime, startTime, endTime]);
