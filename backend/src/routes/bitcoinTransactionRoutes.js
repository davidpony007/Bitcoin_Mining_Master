...
router.get('/summary', async (req, res) => {
  try {
    ...
    const [summary] = await sequelize.query(
      `SELECT 
        COUNT(*) as totalCount,
        SUM(CASE WHEN transaction_type IN ('ad free contract', 'daily sign-in free contract', 'invitation free contract', 'paid contract') THEN transaction_amount ELSE 0 END) as totalMiningReward,
        ...
        COUNT(CASE WHEN transaction_type IN ('ad free contract', 'daily sign-in free contract', 'invitation free contract', 'paid contract') THEN 1 END) as miningCount,
...
function getTypeLabel(type) {
  const labels = {
    'ad free contract': 'Ad Reward',
    'daily sign-in free contract': 'Daily Check-in',
    'invitation free contract': 'Invitation Reward',
